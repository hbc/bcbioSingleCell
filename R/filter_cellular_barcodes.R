...
#' @param reads Minimum number of read counts per cell.
...
filter_cellular_barcodes <- function(
    ...
    run$metrics <- run$metrics %>%
        filter(.data$total_counts > !!total_counts,
               .data$coding_counts > !!coding_counts,
    ...
    if (isTRUE(show)) {
        writeLines(c(
            ...
            "",
            paste0("- `> ", total_counts, "` total read counts per cell"),
            paste0("- `> ", coding_counts, "` coding read counts per cell"),
        ...
        plot_read_counts(
            run, total_min = total_counts, coding_min = coding_counts)
