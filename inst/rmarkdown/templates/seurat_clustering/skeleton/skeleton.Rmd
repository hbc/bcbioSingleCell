...
# Load the bcbio object
bcbName <- load(params$bcbFile)
bcb <- get(bcbName, inherits = FALSE)

# Check to see if seurat object exists
if (!exists("seurat", inherits = FALSE)) {
    seuratFile <- str_replace(params$bcbFile, "\\.rda$", "_seurat.rda")
    if (file.exists(seuratFile)) {
        seuratName <- load(seuratFile)
        seurat <- get(seuratName, inherits = FALSE)
        seuratPreregressFile <- str_replace(seuratFile, "\\.rda$", "_preregress.rda")
        seuratPreregressName <- load(seuratPreregressFile)
        seuratPreregress <- get(seuratPreregressName, inherits = FALSE)
        evalSeurat <- FALSE
    } else {
        evalSeurat <- TRUE
    }
...
seurat <- CellCycleScoring(
    ...
    g2m.genes = g2mGenes,
    s.genes = sGenes)
assignAndSaveData(paste(bcbName, "seurat", "preregress", sep = "_"), seurat)
...
```{r cell_cycle_pca_1, eval=evalSeurat, include=evalSeurat}
RunPCA(
    seuratPreregress,
    pc.genes = c(sGenes, g2mGenes),
    do.print = FALSE) %>%
    PCAPlot(cols.use = viridis(3),
            group.by = "Phase")
```
...
```{r qc_plots_2, results="asis"}
plotFeatures(
    seurat,
    c("nUMI", "nGene", "mitoRatio", "PC"),
    combine = TRUE)
```
