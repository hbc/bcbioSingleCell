...
filteredName <- load(params$filtered)
filtered <- get(filteredName, inherits = FALSE)
rm(list = filteredName)
if (!is(filtered, "bcbioSCFiltered")) {
    stop("bcbioSCFiltered class object required")
}

# Useful metadata slots
interestingGroups <- metadata(filtered)$interestingGroups
organism <- metadata(filtered)$organism
gene2symbol <- metadata(filtered)$gene2symbol
...
# knitr arguments
opts_chunk$set(
    cache.path = paste0(params$filteredName, "Seurat", "_cache/"),
    fig.path = paste0(params$filteredName, "Seurat", "_files/"))
```
...
# Initialize Seurat (`r params$filteredName`)
...
ccm <- str_match(organism, "^([A-Z])[a-z]+ ([a-z]+)$") %>%
    .[, 2:3] %>%
    as.character %>%
    paste0(collapse = "") %>%
    tolower %>%
...
# Cell-cycle `Phase` column will be added to `seurat@meta.data`
