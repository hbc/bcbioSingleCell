...
params:
    bcb_file: "data/bcb_filtered.rda"
    seurat_name: "seurat"
    pc_compute: 20
    pc_use: FALSE
...
assert_is_all_of(bcb, "bcbioSingleCell")
...
mdHeader("S phase markers", level = 3)
s_genes <- cell_cycle_markers %>%
    filter(phase == "S") %>%
    pull("symbol")
print(s_genes)

mdHeader("G2/M phase markers", level = 3)
...
if (!is.numeric(params$pc_use)) {
    pc_use <- plotPCElbow(seurat, maxPct = 0.05, minCumPct = 0.9)
}
```
...
lapply(seq_along(group_by), function(a) {
    mdHeader(group_by[[a]], level = 2, asis = TRUE, tabset = TRUE)
    mdHeader("tSNE", level = 3, asis = TRUE)
    plotTSNE(
        seurat,
        interestingGroups = group_by[[a]],
        label = TRUE) %>%
        show()
