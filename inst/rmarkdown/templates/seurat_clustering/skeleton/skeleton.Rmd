...
# Run non-linear dimensional reduction {.tabset}
...
Advanced, recommended for large datasets. See the [Seurat Mouse Cell Atlas vignette](https://satijalab.org/seurat/mca.html) for more details.

```{bash install_fitsne, eval=FALSE}
# http://fftw.org
brew install fftw
...
RunTSNE(
    ...
    fast_tsne_path = "~/R/software/FIt-SNE/bin/fast_tsne", 
    max_iter = 2000
)
```
...
```{bash conda_install, eval=FALSE}
conda create --name=umap umap-learn
```
...
invisible(lapply(group_by, function(group) {
    markdownHeader(group, level = 2, asis = TRUE, tabset = TRUE)
    markdownHeader("tSNE", level = 3, asis = TRUE)
    plotTSNE(
        object = seurat,
        ...
        show()
    markdownHeader("PCA", level = 3, asis = TRUE)
    plotPCA(
        object = seurat,
        interestingGroups = group,
        label = TRUE
...
Note that t-SNE is not PCA! The measurement of distance in a t-SNE plot is difficult to interpret, and is most helpful for the relationships of close neighbors. To better infer separation distance between the putative clusters, let's reapply PCA.
...
# Alternatively can use `Seurat::FeaturePlot()`, but it doesn't currently
# support labeling the clusters
plotFeatureTSNE(
    object = seurat,
    features = c(
        "nUMI",
