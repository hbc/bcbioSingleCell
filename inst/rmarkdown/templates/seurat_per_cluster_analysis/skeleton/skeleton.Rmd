...
# Load seurat object
seuratName <- load(params$seurat_file)
seurat <- get(seuratName, inherits = FALSE)
assert_is_all_of(bcb, "seurat")
...
invisible(lapply(groups, function(group) {
    mdHeader(group, level = 2, asis = TRUE)
...
invisible(lapply(idents, function(ident) {
    ...
    mdHeader(ident, level = 2, asis = TRUE, tabset = TRUE)
    invisible(lapply(groups, function(group) {
        mdHeader(group, level = 3, asis = TRUE)
