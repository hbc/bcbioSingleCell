...
if (!is.null(params$cellTypeMarkersFile)) {
    cellTypeMarkers <- readFileByExtension(params$cellTypeMarkersFile)
} else {
    # Detect the organism from the seurat object
    organism <- slot(seurat, "misc") %>%
        .[["bcbio"]] %>%
        .[["organism"]]
    if (is.null(organism)) {
        stop("Organism is not slotted in 'seurat@misc$bcbio$organism'")
    }
    # Reformat organism name to match internal list (e.g. "mmusculus")
    organismShorthand <- organism %>%
        gsub(x = .,
             pattern = "^([A-Z])[a-z]+\\s([a-z]+)$",
             replacement = "\\L\\1\\2",
             perl = TRUE)
    # Load our internal cell type markers
    cellTypeMarkers = get("cellTypeMarkers", asNamespace("bcbioSingleCell"))
    # Check whether the organism is supported
    if (!organismShorthand %in% names(cellTypeMarkers)) {
        stop(paste("No internal markers for", organismShorthand))
    }
    cellTypeMarkers <- cellTypeMarkers[[organismShorthand]]
}

# Attempt to load all detected markers, if already saved. This step is
# useful for regenerating the report when caching is disabled.
seuratAllMarkersName <- paste(seuratName, "markers", sep = "_")
seuratAllMarkersFile <- file.path(dataDir, paste0(seuratAllMarkersName, ".rda"))
if (file.exists(seuratAllMarkersFile)) {
    loadDataAsName(c(markers = seuratAllMarkersFile))
    evalMarkers <- FALSE
} else {
    evalMarkers <- TRUE
}
