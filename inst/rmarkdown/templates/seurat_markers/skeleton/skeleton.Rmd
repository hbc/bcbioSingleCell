...
if (!is.null(params$cellTypeMarkersFile)) {
    ...
    saveData(cellTypeMarkers, dir = dataDir, overwrite = FALSE)
}
...
if (file.exists(seuratAllMarkersFile)) {
    loadDataAsName(c(seuratAllMarkers = seuratAllMarkersFile))
    evalMarkers <- FALSE
} else {
    evalMarkers <- TRUE
}
...
```{r top_markers}
topMarkers <- topMarkers(seuratAllMarkers, n = 5, coding = TRUE)
topMarkers
```
...
```{r plot_known_markers, fig.height=14.5, fig.width=10, results="asis"}
plotKnownMarkers(
    seurat,
