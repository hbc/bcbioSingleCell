...
params:
    subsetID: bcbFiltered
    pcCompute: 20
    pcUse: 10
    varsToRegress: !r c("nUMI", "mitoRatio")
    knownMarkersFile: "meta/known_markers.xlsx"
    outputDir: "."
---
...
# Known markers file
knownMarkersFile <- normalizePath(params$knownMarkersFile)
...
```{r saveSeurat}
assignAndSaveData(paste0(subsetID, "Seurat"), seurat)
```
...
```{r saveMarkers}
assignAndSaveData(paste0(subsetID, "Markers"), markers)
dir.create(markersDir, recursive = TRUE, showWarnings = FALSE)
write_csv(markers, file.path(markersDir, paste0(subsetID, ".csv.gz")))
```
...
```{r plotTopMarkers, fig.height=12, fig.width=12, results="asis"}
...
```{r knownMarkers}
knownMarkers <- readKnownMarkers(knownMarkersFile, genomeBuild, show = TRUE)
...
```{r plotKnownMarkers, fig.height=12, fig.width=12, results="asis"}
