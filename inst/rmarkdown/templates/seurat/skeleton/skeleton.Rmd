...
# Directory paths
markersDir <- normalizePath(outputDir) %>%
    file.path("results", "markers")

# knitr arguments
opts_chunk$set(
    cache = TRUE,
    cache.lazy = FALSE,
    cache.path = paste0(subsetName, "_cache/"),
    fig.path = paste0(subsetName, "_files/"),
    results = "asis")
```
...
```{r saveSeurat}
assignAndSaveData(paste0(subsetName, "_seurat"), seurat)
```
...
```{r saveMarkers}
assignAndSaveData(paste0(subsetName, "_markers"), markers)
dir.create(markersDir, recursive = TRUE, showWarnings = FALSE)
write_csv(markers,
          file.path(markersDir, paste0(subsetName, ".csv.gz")))
```
