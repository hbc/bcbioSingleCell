...
```{r PCA}
seurat <- seurat %>%
    PCAFast(pcs.compute = params$pc_compute, do.print = FALSE) %>%
...
```{r PCElbowPlot}
PCElbowPlot(seurat, num.pc = params$pc_compute) +
...
seurat <- JackStraw(
    seurat,
    num.pc = params$pc_compute,
    do.print = FALSE)
JackStrawPlot(seurat, PCs = 1:params$pc_compute)
```
...
dir.create(params$markers_dir,
           recursive = TRUE,
           showWarnings = FALSE)
write_csv(markers,
          file.path(params$markers_dir, paste0(subset_name, ".csv.gz")))
```
