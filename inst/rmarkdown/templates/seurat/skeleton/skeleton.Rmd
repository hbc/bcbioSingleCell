...
seurat <- new("seurat", raw.data = raw.data) %>%
    ...
    SubsetData(subset.name = "percent.mito", accept.high = percent.mito)
print(seurat)
```

```{r VlnPlot}
VlnPlot(seurat, c("nGene", "nUMI", "percent.mito"), nCol = 3)
```
...
```{r GenePlot}
par(mfrow = c(1, 2))
GenePlot(seurat, "nUMI", "percent.mito")
...
Here, let's regress effects on gene expression that are due to the number of genes detected (`nGene`), the total counts in the cell (`nUMI`), and the percent mitochondrial content (`percent.mito`).

```{r RegressOut}
seurat <- RegressOut(
    seurat,
    latent.vars = c("nGene", "nUMI", "percent.mito"))
```
...
```{r JackStraw, eval=isTRUE(detect_hpc))}
...
seurat <- FindClusters(
    seurat,
    do.sparse = TRUE,  # required for large datasets
    pc.use = 1:pc.use,
    print.output = FALSE,
    save.SNN = TRUE)
```
...
Let's look at the variance in the number of genes detected (`nGene`) and the percentage of mitochondrial gene expression (`percent.mito`), to see if any abberant cells are clustering.

```{r qc}
FeaturePlot(seurat, "nGene", cols.use = c("grey","green"))
FeaturePlot(seurat, "percent.mito", cols.use = c("grey","red"))
```
